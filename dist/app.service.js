...
let AppService = class AppService {
    ...
    sortData() {
        ...
        const itemsSort = [];
        items.map((item) => {
            const { id, parentId } = item, propsItem = __rest(item, ["id", "parentId"]);
            if (parentId && parentId !== 0) {
                const parentIndex = itemsSort.findIndex(itemSort => itemSort.id = parentId);
                if (parentIndex && parentIndex !== -1) {
                    itemsSort[parentIndex].items.push(Object.assign({ id }, propsItem));
                }
                else {
                    itemsSort.push({
                        id: parentId,
                        items: [Object.assign({ id }, propsItem)]
                    });
                }
            }
            else {
                const findIndex = itemsSort.findIndex(itemSort => itemSort.id = id);
                if (findIndex && findIndex !== -1) {
                    itemsSort[findIndex] = Object.assign(Object.assign({}, itemsSort[findIndex]), propsItem);
                }
                else {
                    itemsSort.push(Object.assign(Object.assign({ id }, propsItem), { items: [] }));
                }
            }
        });
        return itemsSort;
