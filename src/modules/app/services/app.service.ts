...
export class AppService {
  ...
  public sortData(filter = false) {
    ...
    items = items.sort((a, b) => b.parentId - a.parentId);

    for (let i = 0; i < items.length; i++) {

      const indexSort = items.findIndex(itemIndex => itemIndex.id === items[i].parentId);

      if (indexSort !== -1) {
        if (!items[indexSort].items) {
          items[indexSort].items = [];
        }

        items[indexSort].items.push(items[i]);
